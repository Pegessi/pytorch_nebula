...
#define DRV_CALL(call)                                                                                  \
    {                                                                                                   \
        ...
        {                                                                                              \
            const char *errMsg; hipGetErrorString(result, &errMsg);                                     \
...
size_t getGranularitySize()
{
    ...
    if(granularity == -1) {
        int current_device;
        DRV_CALL(hipCtxGetDevice(&current_device));
...
hipError_t setMemAccess(void* ptr, size_t size, int current_device_in = -1)
{
    ...
    hipError_t result = cuMemSetAccess((hipDeviceptr_t)ptr, size, &accessDesc, 1); 
    return result;
}
...
struct BlockSegment
{
    ...
    BlockSegment(c10::hip::HIPCachingAllocator::Native::Block* block, size_t offset):block(block), offset(offset) {}
    
    c10::hip::HIPCachingAllocator::Native::Block* block;
    size_t offset;
};
...
struct PhyBlock
{
                                                                            ...
                                                                            released(false)
    {
        if(device_id == -1)
        {
          DRV_CALL(hipCtxGetDevice(&device_id));
        }
...
struct VirDevPtr {
    VirDevPtr(hipDeviceptr_t addr_in, size_t allocSize_in, int device_id = -1): allocSize(allocSize_in), mapped(false), device_id(device_id), status(hipSuccess), released(false) {
        if(device_id == -1) {
            DRV_CALL(hipCtxGetDevice(&device_id));
        }
      
        hipDeviceptr_t device_ptr;
        hipDeviceptr_t request_ptr = addr_in;
      
        DRV_CALL_RET(cuMemAddressReserve(&device_ptr, allocSize, 0ULL, request_ptr, 0ULL), status);
      
        if(status != hipSuccess || (request_ptr != 0ULL && device_ptr != request_ptr)) {
    ...
    void release_resources() {
        ...
        if(virAddr) {
            if(mapped) {
                DRV_CALL(cuMemUnmap((hipDeviceptr_t)virAddr, allocSize));
            }
            DRV_CALL(cuMemAddressFree((hipDeviceptr_t)virAddr, allocSize)); 
        }
...
struct VmmSegment {
    VmmSegment():granul_size(0), segment_ptr(nullptr), status(hipSuccess), free_blocks(0), released(false) {}
    ...
    void allocate_phy_blocks(size_t blocks, size_t block_size_in, int device_id_in) {
        ...
        for(size_t i=0; i<blocks; i++)
        {
            ...
            if(phy_block->status != hipSuccess) {
                size_t device_free;
                size_t device_total;
    ...
    void* mapVirAddr() {
        ...
        hipDeviceptr_t device_ptr = 0ULL;
        ...
        do
        {
            ...
            device_ptr = (hipDeviceptr_t)vir_dev_ptr->virAddr;
            
            if(vir_dev_ptr->status != hipSuccess || !vir_dev_ptr->virAddr) {
                ...
                for(size_t j = 0; j < phy_blocks.size(); j++) {
                    ...
                    if(vir_block->status != hipSuccess) {
            ...
            current_try++;
            device_ptr = 0ULL;
        } while(result != hipSuccess && current_try < retry_times);    
