...
// #include <c10/hip/hip_vmm_allocator.h>
// #endif
...
static const bool log_hipAPI = ([]() -> bool {
    const char* env = getenv("LOG_HIPAPI");
...
size_t hipMalloc_counts = 0, hipFree_counts = 0;
void DTRLogcudaAPIEvents(const std::string& name, size_t size, int64_t addr) {
  std::string log_msg = "{";
  log_msg += "\"TYPE\":\"" + name + "\", ";
  log_msg += "\"COUNTS\":\"" + (name=="hipMalloc" ? std::to_string(++hipMalloc_counts) : std::to_string(++hipFree_counts)) + "\", ";
  log_msg += "\"SIZE\":" + std::to_string(size) + ", ";
  log_msg += "\"ADDR\":" + std::to_string(addr);
  log_msg += "}";
  DTRLogger::logger().log(log_msg);
...
{
  EventIDCounter(hipStream_t stream):stream(stream), current_event_id(0) {}
...
static std::unordered_map<hipStream_t, std::shared_ptr<EventIDCounter>> stream_id_counter;
static std::mutex counter_mutex;
struct BlockEvent {
  BlockEvent(hipStream_t stream_in, bool record_event=false) {
    stream = stream_in;
  ...
  void record(hipStream_t stream_in) {
    ...
    if(stream == stream_in)
    {
      ...
      
      {
        ...
        C10_HIP_CHECK(hipEventRecord(event, stream));
      }
    }
  }
  ...
  static void event_gc(hipStream_t stream, uint64_t event_id, hipEvent_t event)
  {
    using EventQueue=std::map<uint64_t, hipEvent_t>;
    
    static std::unordered_map<hipStream_t, EventQueue> event_pool;
...
struct SegmentTwin {
  ...
  bool empty();
  inline Block* flush_cost(time_t cur, size_t need_size, hipStream_t stream);
};
...
*/
inline Block* SegmentTwin::flush_cost(time_t cur, size_t need_size, hipStream_t stream) {
...
public:
  ...
  bool auto_evict(size_t need_size, int device, hipStream_t stream) {
          ...
          if(res.second) {
            if(auto sap = res.first.lock()) {
              if(sap->evictable()){
                ...
                //   if(it->lock()){
                //     pm->record_evicted_tensors(c10::hip::current_device(), (*it));
                //     break;
                //   }
                // }
              #endif
...
// unallocated cached blocks larger than 64 MB
  ...
  std::unordered_map<hipStream_t, BlockEventOrderPool> free_fused_blocks_in_release_order;
  
  // fused blocks which is free, but it's phy_blocks are used by other block of my stream
  std::unordered_map<hipStream_t, BlockEventOrderPool> fragmented_free_fused_blocks;
#endif
...
//       C10_HIP_CHECK(hipStreamGetCaptureInfo(stream, &status, &id));
...
// #endif
    ...
    for(auto& it : fragmented_free_fused_blocks) {
      for(auto block_it = it.second.blocks.begin(); block_it != it.second.blocks.end();) {
        ...
        if(block->self_last_event) {
          err = hipEventQuery(block->self_last_event->event);
        }
  ...
  bool get_fused_fragmented_blocks(AllocParams& p, int time) {
    ...
    if (p.pool->is_small || p.search_key.size < fragment_limit) {
      ...
      do
      {
        ...
        if(vmm_segment->status == hipSuccess && vmm_segment->segment_ptr) {
          break;
        } else {
          hipGetLastError();
      ...
      if(!vmm_segment || vmm_segment->status != hipSuccess || !vmm_segment->segment_ptr) {
          return false;
      }
  ...
  bool alloc_block(
    ...
    if (set_fraction &&
      ...
      p.err = hipMallocMaybeCapturing(&ptr, size);
      #ifdef MEM_EVENTS_REC
      if(log_hipAPI) DTRLogcudaAPIEvents("hipMalloc", size, reinterpret_cast<uintptr_t>(ptr));
      #endif
      if (p.err != hipSuccess) {
        if (p.err == hipErrorMemoryAllocation) {
...
static const int vmmDefragment = ([]()->int{
...
#ifdef MEM_EVENTS_REC
    if(log_hipAPI) DTRLogcudaAPIEvents("hipFree", block->size, reinterpret_cast<uintptr_t>(block->ptr));
#endif
#ifdef MEM_TWIN_REC
    if(UNIFIED_EVICT){
  ...
  void insert_free_event_into_alloc_stream(Block* block)
  {
    ...
    {
      C10_HIP_CHECK(hipSetDevice(block->device));
    }
  ...
  DataPtr allocate(size_t size) const override {
    ...
    if (size != 0) {
      ...
      const_cast<NativeCachingAllocator*>(this)->malloc(
          &r, device, size, hip::getCurrentHIPStream(device));
    }
